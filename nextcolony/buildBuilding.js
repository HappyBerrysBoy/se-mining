  ...
  .then(planets => {
    planets.forEach(planet => {
      axios
        ...
        .then(
          axios.spread(
            (qytInfoData, prodInfoData, buildingInfoData, shipyardInfoData) => {
...
axios
  .all([
    loadplanets(account),
    loadqyt(planetId),
    loadproduction(planetId, account),
    loadbuilding(planetId),
  ]) // axios.all로 여러 개의 request를 보내고
  .then(
    axios.spread((planetInfo, qytInfo, prodInfo, buildingInfo) => {
      // response를 spread로 받는다
      console.log(planetInfo, qytInfo, prodInfo, buildingInfo);
    }),
  )
  .catch(error => {
    console.error(error);
