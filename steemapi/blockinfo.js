...
bot.on('message', msg => {
  ...
  bot.sendMessage(chatId, `반갑습니다. ${msg.from.first_name}님. 메아리(${msg.text})`);
});
...
const fleetMission = account => {
  return axios.get(`https://api.nextcolony.io/loadfleetmission?user=${account}&active=1`);
};
...
const findPlanet = () => {
  return axios.get(`https://api.nextcolony.io/loadplanets?sort=date&from=0&to=1`);
};
  ...
  transactions.forEach(async transaction => {
    ...
    if (action === 'custom_json') {
      try {
        ...
        if (useTelegramBot && content.id == 'nextcolony') {
          ...
          if (jsonInfo.type == 'siege' || jsonInfo.type == 'attack') {
            ...
            const missions = await fleetPlanetMission(jsonInfo.username, planetid);
            ...
            for (let i = 0; i < ncUsers.telegramMembers.length; i++) {
              ...
              if (targetAccountList.includes(targetPlanetInfo.data.planets[0].user)) {
                let arrTime = 0;
                let retTime = 0;
                ...
                if (thisMission) {
                  ...
                  sendMsg = `====비상비상====\nAccount:${jsonInfo.username}  \nType:${
                    ...
                    jsonInfo.command.tr_var1,
                  )}  \nFrom:${planetid}\nTo:${targetPlanetInfo.data.planets[0].user}(${
            ...
            if (username != 'rivalzzz' && username != 'mancer-sm-alt') {
              ...
              if (sendMsg) {
                ...
                yamatobot.sendMessage(6151083556, sendMsg); // 454924368 : youthme
      ...
      try {
        ...
        let options = content.body.split(config.diceTag)[1].split('\n')[0].trim();
        ...
        const theNumber = Math.floor(Math.random() * (endNo - startNo + 1)) + startNo;
